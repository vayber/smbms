...
$(function(){
	...
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/providerManagement.do?method=query&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/providerManagement.do?method=modifyQuery&proid="+ obj.attr("proid");
	});
	...
	$(".deleteProvider").on("click",function(){
		...
		if(confirm("你确定要删除供应商【"+obj.attr("proname")+"】吗？")){
			$.ajax({
				type:"GET",
				url:path+"/providerManagement.do?method=delete",
